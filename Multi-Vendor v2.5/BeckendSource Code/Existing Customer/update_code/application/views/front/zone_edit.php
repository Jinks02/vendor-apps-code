...
.input-color .color-box {
      ...
      <script type="text/javascript">
         ...
         var colorButtons = {};
          var coordinates1= [];
                  ...
                  var corrdinates;
           <?php $zone = $this->db->get_where('zone', array('zone_id' => $zone_id))->row()->original_position  ?>;
          corrdinates = <?php echo $zone ?>;
             javaArray =  corrdinates.split(',');
           ...
           if (typeof selectedShape.getPath == 'function') {
             ...
             for (var i = 0; i < selectedShape.getPath().getLength(); i++) {
               ...
               pathstr += selectedShape.getPath().getAt(i).toUrlValue() + " , ";
                 coordinates1.push(selectedShape.getPath().getAt(i).toUrlValue());
         ...
         function initialize() {
           ...
           map = new google.maps.Map(document.getElementById('map'), { //var
             zoom: 12,//10,
             center: new google.maps.LatLng(javaArray[i], javaArray[i+1]),//(22.344, 114.048),
      ...
      drawingModes: [
        google.maps.drawing.OverlayType.POLYGON
             ...
             map: map
              ...
              var triangleCoords  = [];
              var i=0;
             var limit = javaArray.length;
             for(ban of javaArray){
             
                 if(i%2==0 && i<limit ){
                     
               triangleCoords.push(new google.maps.LatLng(javaArray[i], javaArray[i+1]));
  ...
  selectedShape = new google.maps.Polygon({
    paths: triangleCoords, // turn off if it gets annoying
    clickable:true,
    strokeColor: '#FF0000',
    strokeOpacity: 0.8,
    strokeWeight: 2,
  ...
  selectedShape.setMap(map);
   ...
   <body class="mainbody">
      ...
      <div id="panel">
         ...
         <h4 style="text-align:center;"><b>Edit Zone</b></h4>
         <?php $zone_name = $this->db->get_where('zone', array('zone_id' => $zone_id))->row()->title; ?>
         <input type="text" id="title" value="<?php echo $zone_name; ?>" class="margintop10 width100" style="  border: none;
            border-bottom: 1px solid grey;margin-bottom: 10px;outline:0;font-weight:600;" placeholder="Zone name">
         <div id="color-palette" ></div>
          <?php $zone_position = $this->db->get_where('zone', array('zone_id' => $zone_id))->row()->position_array; ?>
         <textarea id="cursel" readonly colls="100" style="height:100px;margin-bottom:20px;" class="margintop10 width100" row="30">
        <?php echo $zone_position; ?>
          </textarea>
          <input type="hidden" id="coordinates" name="coordinates[]" >
         <br>
         <button type="button" id="popmodal" onClick="saveData(<?php echo $zone_id; ?>)" class="btn btn-primary" style="margin:auto;display:block;width:134px;border:none;outline:0;">Save</button>
         <br>
          <P>Draw new zone to add or edit your current zone</p>
          <div class="input-color">
          <P> Your Current Zone</p>
...
<script>
   ...
   function saveData(id){
       ...
       $.ajax({
               type: 'post',
               url: '<?php echo base_url(); ?>index.php/Zone/saveZone/edit',
               data: ({position: position, title: title, coordinates: coordinates, zone_id: id}),
               dataType: 'html', 
               beforeSend: function() {
                    popupload();
                   },
